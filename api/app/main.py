...
app = FastAPI(title="e-GOV Login API", version="8.0.0")
...
def init_db() -> None:
    with db(immediate=True) as con:
        ...
        if table_exists(con, "students"):
            con.execute(
                """
                INSERT OR IGNORE INTO people(cpf, name, role, active, created_at, updated_at)
                SELECT cpf, name, 'aluno', active, created_at, updated_at
                FROM students
                """
            )
...
def get_person(con: sqlite3.Connection, cpf: str) -> sqlite3.Row | None:
    return con.execute(
        """
        SELECT cpf, name, role, active
        FROM people
        WHERE cpf=?
        """,
        (cpf,),
...
def active_sessions_for_cpf(con: sqlite3.Connection, cpf: str) -> list[sqlite3.Row]:
    return con.execute(
        """
        SELECT *
        FROM sessions
...
def auth_cpf(
    ...
    if not valid_cpf(cpf):
        return auth_denied("cpf_invalido", "CPF invalido.")
    ...
    with db(immediate=True) as con:
        ...
        if person is None:
            return auth_denied("nao_cadastrado", "CPF nao cadastrado.")
        ...
        cpf_sessions = active_sessions_for_cpf(con, cpf)

        if cpf_sessions:
            same_pc = next(
                (s for s in cpf_sessions if s["computer"].casefold() == computer.casefold()),
                None,
            )
            ...
            other = cpf_sessions[0]
            return auth_denied(
                "cpf_already_online",
                f"CPF ja conectado em {other['computer']}.",
...
def list_people(
    ...
    with db() as con:
        rows = con.execute(
            """
            SELECT cpf, name, role, active, created_at, updated_at
            FROM people
            ORDER BY role, name, cpf
            """
        ).fetchall()
